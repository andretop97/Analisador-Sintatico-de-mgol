    ...
    def analyzer(self):
        ...
        while True:
            ...
            if actionResult[0] == "s":
                ...
                derivation = self.semanticAnalyzer.analyzer(indice + 1, self.symbols[token - len(rules[indice][1]): token])
                for i in range(len(rules[indice][1])):
                    stack.pop()
                    self.symbols.pop(indice)
                self.symbols.insert(token, derivation)
                state = stack[-1]
                ...
                if self.symbols[token]["token"] in Dicionario_de_erros:
                    ...
                    if self.symbols[token]["token"] == "1":
                        self.symbols.insert(token, {"lexema": self.symbols[token]["lexema"], "token": "id", "tipo": "",
                                                    "line": self.symbols[token]["line"],
                                                    "column": self.symbols[token]["column"]})
                        self.symbols.pop(token + 1)
                        print("\n\n", Dicionario_de_erros["1"], "\"", self.symbols[token]
                        ["lexema"], "\" ", "na linha ", self.symbols[token]["line"], "\n\n")
                        ...
                        self.symbols.insert(token, {"lexema": self.symbols[token]["lexema"], "token": "Literal",
                                                    "tipo": "", "line": self.symbols[token]["line"],
                                                    "column": self.symbols[token]["column"]})
                        self.symbols.pop(token + 1)
                        print("\n\n", Dicionario_de_erros["3"], "\"", self.symbols[token]
                        ["lexema"], "\" ", "na linha ", self.symbols[token]["line"], "\n\n")

                    elif self.symbols[token]["token"] == "4":
                        self.symbols.insert(token, {"lexema": self.symbols[token]["lexema"], "token": "Comentário",
                                                    "tipo": "", "line": self.symbols[token]["line"],
                                                    "column": self.symbols[token]["column"]})
                        self.symbols.pop(token + 1)
                        print("\n\n", Dicionario_de_erros["4"], "\"", self.symbols[token]
                        ["lexema"], "\" ", "na linha ", self.symbols[token]["line"], "\n\n")

                elif self.symbols[token]["token"] == "Comentário":
                    token += 1

                # tratamento geral para identificar os erros
                elif actionResult[0] == "Err":

                    if str(actionResult[1]) == "5":  # Faltou início
                        self.symbols.insert(
                            token, {"lexema": "inicio", "token": "inicio", "tipo": "", "line": 1, "column": 0})
                        print("\n\n", Dicionario_de_erros[str(
                            actionResult[1])], "na linha ", self.symbols[token]["line"], "\n\n")

                    elif str(actionResult[1]) == "6":  # Faltou varinício
                        self.symbols.insert(token, {"lexema": "varinicio", "token": "varinicio", "tipo": "",
                                                    "line": self.symbols[token - 1]["line"],
                                                    "column": self.symbols[token - 1]["column"]})
                        print("\n\n", Dicionario_de_erros[str(actionResult[1])], "entre as linhas",
                              self.symbols[token - 1]
                              ["line"], " e ", self.symbols[token + 1]["line"], self.symbols[token - 1]["column"],
                              "\n\n")
                        ...
                        self.symbols.insert(token, {"lexema": ";", "token": "PT_V", "tipo": "",
                                                    ...
                                                    "column": self.symbols[token - 1]["column"]})
                        print("\n\n", Dicionario_de_erros[str(
                            actionResult[1])], "na linha", self.symbols[token - 1]["line"], " e coluna ",
                              self.symbols[token - 1]["column"], "\n\n")
                    ...
                    # Faltou operador de atribuição
                    elif str(actionResult[1]) == "11":
                        stack.append(29)
                        print("\n\n", Dicionario_de_erros[str(
                            actionResult[1])], "na linha", self.symbols[token - 1]["line"], " e coluna ",
                              self.symbols[token - 1]["column"], "\n\n")
                        ...
                        self.symbols.insert(token, {"lexema": "entao", "token": "entao", "tipo": "",
                                                    "line": self.symbols[token - 1]["line"],
                                                    "column": self.symbols[token - 1]["column"]})
                        print("\n\n", Dicionario_de_erros[str(actionResult[1])], "na linha", int(
                            self.symbols[token - 1]["line"]) + 1, " e coluna ", self.symbols[token - 1]["column"],
                              "\n\n")
                        ...
                        self.symbols.insert(token, {"lexema": "fim", "token": "fim", "tipo": "",
                                                    "line": self.symbols[token - 1]["line"],
                                                    "column": self.symbols[token - 1]["column"]})
                        print("\n\n", Dicionario_de_erros[str(
                            actionResult[1])], "na linha", self.symbols[token - 1]["line"], "\n\n")
                        ...
                        self.symbols.insert(token, {"lexema": "fimse", "token": "fimse", "tipo": "",
                                                    "line": self.symbols[token - 1]["line"],
                                                    "column": self.symbols[token - 1]["column"]})
                        print("\n\n", Dicionario_de_erros[str(
                            actionResult[1])], "na linha", self.symbols[token - 1]["line"], " e coluna ",
